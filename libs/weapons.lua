...
Weapons.list = {
    ...
    Shotgun = {
        ...
        damage = 20,
        bullets = function() return 3 + math.random(2) end,  -- Number of bullets shot at once
    },
    Rifle = {
        name = "Rifle",
        number = 2,
...
function Weapons.Shoot(player, Entities, weapon)
    ...
    if love.timer.getTime() > player.NextShoot then
        ...
        repeat
            ...
            local angle = player.angle + (math.random(-weapon.spread * 100, weapon.spread * 100) / 100) * player.fov / math.pi
